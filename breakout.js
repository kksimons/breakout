...
document.addEventListener("mousemove", (e) => {
    ...
    if(val > player.w && val < canvas.width) {player.x = val - player.w}
    if(!game.inplay) {ball.x = val - (player.w / 2);}
})

function movement() {
    if(keyz.ArrowLeft) {player.x -= player.speed;}
    if(keyz.ArrowRight) {player.x += player.speed}
    if(!game.inplay) {ball.x = player.x + player.w / 2};
}
...
function draw() {
    ...
    if (game.bricks.length <= 0) {gameWinner();}
    }
    })
    ...
    if(game.gameover) {   
        ctx.font = "24px serif";
        output = `GAME OVER you hit ${player.score}`
    }
    if (canvas.width < 900) {
        ctx.font = "20px serif";
    }
...
function startGame() {
    ...
    let yy = 0;
    for(let i = 0; i < game.bricksnum; i++) {
        if(i % totalAcross == 0) {
            yy ++;
            yPos += height + buffer;
            xPos = game.grid / 2;
